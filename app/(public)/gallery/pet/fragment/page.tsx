...
export default function PetFragmentPage() {
    const allItems = gallery_pet_fragment_data as unknown as FragmentItem[];
        ...
        <GalleryShell
            ...
            empty={
                <div className="border-border text-muted-foreground flex flex-col items-center justify-center rounded-xl border border-dashed py-16">
                    <p className="text-lg">未找到匹配的宠物碎片</p>
                    <p className="mt-1 text-sm">请尝试其他关键词搜索</p>
                </div>
            }>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {filteredItems.map((item) => (
                    <FragmentCard key={item.id} item={item} />
                ))}
            </div>
        </GalleryShell>
    );
