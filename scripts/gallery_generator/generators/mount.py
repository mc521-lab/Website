...
name = "mount"
help = "从 MCPets/Pets 和 MMOItems/item/material.yml 生成坐骑 gallery 数据"
...
FRAGMENT_KEY_RE = re.compile(r"^ZQ_[A-Z0-9_]+$")
...
def build_material_fragment_content(key: str, base: dict[str, Any]) -> dict[str, Any]:
    ...
    for line in cleaned:
        ...
        if "集齐" in line:
            ...
            continue
        if "即可兑换坐骑" in line:
            line = line.replace("即可兑换坐骑-", "用于兑换坐骑")
            line = line.strip()
            if line and line not in usage:
                usage[0] += line
    return {
...
def run(src: Path, dst: Path) -> int:
    """从 MCPets/Pets 和 MMOItems/item/material.yml 生成坐骑 gallery 文件。"""
    ...
    if petsrc.exists():
        ...
        for file in files:
            if "定制" in file.parts:
                continue
            if "坐骑" not in file.parts:
                continue
            if not any([level in file.parts for level in ["C级宠物", "B级宠物", "A级宠物", "S级宠物"]]):
                continue
            if any([part in file.parts for part in ["幽冥", "红色角暴龙"]]):
                continue
